      ...
      <?= $arResult["FORM_FOOTER"]?>
        ...
        <script>
		  ...
		  document.querySelector("<?=$arParams['PARENT_CLASS']?>   form[name='" + "<?=$arResult['arForm']['SID']?>" + "'] input[placeholder='TITLE']").value = document.title;
			...
			let parentForm03 = document.querySelector("<?=$arParams['PARENT_CLASS']?>   form[name='" + "<?=$arResult['arForm']['SID']?>" + "']");
			parentForm03.querySelector('.modal__btn-repeat').addEventListener('click', function () {
				parentForm03.querySelector('.modal__wrap-fields').style.display = '';
				parentForm03.querySelector('.modal__btn-submit').style.display = '';
				parentForm03.querySelector('.checkbox--privacy-policy').style.display = '';
				let inputs = parentForm03.querySelectorAll('.input__field');
				inputs.forEach(function(element) {
                  element.value = '';
                });
				let inputsarea = parentForm03.querySelectorAll('.textarea__field');
				inputsarea.forEach(function(element) {
                  element.value = '';
                });
				parentForm03.querySelector('.success-msg').innerHTML = "";
				parentForm03.querySelector('.modal__btn-repeat').style.display = 'none';
			});
		  const newInput3 = document.createElement('div');
		  newInput3.style.display = 'none'; 
          newInput3.innerHTML = '<input type="text" name="anti" value="" style="visability:hidden; height: 0; width: 0; padding: 0; border:none;"/>';
          document.querySelector("<?=$arParams['PARENT_CLASS']?>   form[name='" + "<?=$arResult['arForm']['SID']?>").appendChild(newInput3);
        </script>
