        ...
        <?= $arResult["FORM_HEADER"] ?>
          ...
          <div class="modal__wrap-content">
            ...
            <div class="modal__wrap-fields">
				...
				<?=$arResult['funcGetInputHtml']($arResult["QUESTIONS"]['SIMPLE_01'], $arResult['arrVALUES'])?>
				<?=$arResult['funcGetInputHtml']($arResult["QUESTIONS"]['SIMPLE_02'], $arResult['arrVALUES'])?>
				<?=$arResult['funcGetInputHtml']($arResult["QUESTIONS"]['SIMPLE_03'], $arResult['arrVALUES'])?>
				<?=$arResult['funcGetInputHtml']($arResult["QUESTIONS"]['SIMPLE_04'], $arResult['arrVALUES'])?>
				<?=$arResult['funcGetInputHtml']($arResult["QUESTIONS"]['SIMPLE_05'], $arResult['arrVALUES'])?>
				<?=$arResult['funcGetInputHtml']($arResult["QUESTIONS"]['SIMPLE_06'], $arResult['arrVALUES'])?>
				<?=$arResult['funcGetInputHtml']($arResult["QUESTIONS"]['SIMPLE_07'], $arResult['arrVALUES'])?>
				<?=$arResult['funcGetInputHtml']($arResult["QUESTIONS"]['SIMPLE_08'], $arResult['arrVALUES'])?>
				<?=$arResult['funcGetInputHtml']($arResult["QUESTIONS"]['SIMPLE_09'], $arResult['arrVALUES'])?>
				<?=$arResult['funcGetInputHtml']($arResult["QUESTIONS"]['SIMPLE_10'], $arResult['arrVALUES'])?>
				<?=$arResult['funcGetInputHtml']($arResult["QUESTIONS"]['SIMPLE_11'], $arResult['arrVALUES'])?>
				<?=$arResult['funcGetInputHtml']($arResult["QUESTIONS"]['SIMPLE_12'], $arResult['arrVALUES'])?>
				<?=$arResult['funcGetInputHtml']($arResult["QUESTIONS"]['SIMPLE_13'], $arResult['arrVALUES'])?>
				<?=$arResult['funcGetInputHtml']($arResult["QUESTIONS"]['SIMPLE_14'], $arResult['arrVALUES'])?>
				<?=$arResult['funcGetInputHtml']($arResult["QUESTIONS"]['SIMPLE_15'], $arResult['arrVALUES'])?>
      ...
      <?= $arResult["FORM_FOOTER"]?>
        ...
        <script>
		  ...
		  document.querySelector("<?=$arParams['PARENT_CLASS']?>   form[name='" + "<?=$arResult['arForm']['SID']?>" + "'] input[placeholder='TITLE']").value = document.title;
			...
			let parentForm07 = document.querySelector("<?=$arParams['PARENT_CLASS']?>   form[name='" + "<?=$arResult['arForm']['SID']?>" + "']");
			parentForm07.querySelector('.modal__btn-repeat').addEventListener('click', function () {
				parentForm07.querySelector('.modal__wrap-fields').style.display = '';
				parentForm07.querySelector('.modal__btn-submit').style.display = '';
				parentForm07.querySelector('.checkbox--privacy-policy').style.display = '';
				let inputs = parentForm07.querySelectorAll('.input__field');
				inputs.forEach(function(element) {
                  element.value = '';
                });
				parentForm07.querySelector('.success-msg').innerHTML = "";
				parentForm07.querySelector('.modal__btn-repeat').style.display = 'none';
			});
		  const newInput7 = document.createElement('div');
		  newInput7.style.display = 'none'; 
          newInput7.innerHTML = '<input type="text" name="anti" value="" style="visability:hidden; height: 0; width: 0; padding: 0; border:none;"/>';
          document.querySelector("<?=$arParams['PARENT_CLASS']?>   form[name='" + "<?=$arResult['arForm']['SID']?>").appendChild(newInput7);
        </script>
