      ...
      <?= $arResult["FORM_FOOTER"]?>
        ...
        <script>
		  ...
		  document.querySelector("<?=$arParams['PARENT_CLASS']?>   form[name='" + "<?=$arResult['arForm']['SID']?>" + "'] input[placeholder='TITLE']").value = document.title;
			...
			let parentForm01 = document.querySelector("<?=$arParams['PARENT_CLASS']?>   form[name='" + "<?=$arResult['arForm']['SID']?>" + "']");
			parentForm01.querySelector('.modal__btn-repeat').addEventListener('click', function () {
				parentForm01.querySelector('.modal__wrap-fields').style.display = '';
				parentForm01.querySelector('.modal__btn-submit').style.display = '';
				parentForm01.querySelector('.checkbox--privacy-policy').style.display = '';
				let inputs = parentForm01.querySelectorAll('.input__field');
				inputs.forEach(function(element) {
                  element.value = '';
                });
				let inputsarea = parentForm01.querySelectorAll('.textarea__field');
				inputsarea.forEach(function(element) {
                  element.value = '';
                });
				parentForm01.querySelector('.success-msg').innerHTML = "";
				parentForm01.querySelector('.modal__btn-repeat').style.display = 'none';
			});
		  const newInput1 = document.createElement('div');
		  newInput1.style.display = 'none'; 
          newInput1.innerHTML = '<input type="text" name="anti" value="" style="visability:hidden; height: 0; width: 0; padding: 0; border:none;"/>';
          document.querySelector("<?=$arParams['PARENT_CLASS']?>   form[name='" + "<?=$arResult['arForm']['SID']?>").appendChild(newInput1);
        </script>
